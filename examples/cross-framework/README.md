...
Two agents built with different frameworks discover and call each other via the Inai mesh.
...
Both agents run in **demo mode** by default — tool functions return realistic mock data, no LLM calls are made. The full Inai plumbing runs: capability extraction, discovery registration, `AgentClient` lookup, and A2A request/response.
...
### Key Inai concepts shown

- **Plugin wrapping** — `wrap_google_adk()` and `wrap_crewai()` adapt framework agents to `IAgent` in one call
- **Capability extraction** — tools (`FunctionTool`, `@crewai_tool`) automatically become Inai capabilities
...
├── research_agent.py   Google ADK agent definition + Inai wrapping
├── writer_agent.py     CrewAI agent definition + Inai wrapping
...
```bash
cd /path/to/your/inai-project
inai create agent FactCheckerAgent -c fact_check,find_sources --framework langgraph
```
