...
fn generate_test(project_dir: &Path, lang: &Lang, agent_name: &str) -> Result<()> {
    ...
    logger::dim(&format!("Run with: inai test {agent_name}"));
    Ok(())
}
...
fn run_tests(
    ...
    println!(
        "\n{}",
        format!("Running Inai tests ({lang}) — {label}").bold()
    );
...
fn ts_test_template(name: &str) -> String {
    format!(
        r#"/**
 * Unit tests for {name}
 * Generated by: inai test --generate {name}
 */
...
async def test_handle_request(agent, cap):
    req = AgentRequest(request_id="t1", from_id="inai://test",
...
async def test_unknown_capability(agent):
    req = AgentRequest(request_id="t2", from_id="inai://test",
