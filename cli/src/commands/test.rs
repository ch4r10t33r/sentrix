...
pub fn run(args: TestArgs) -> Result<()> {
    ...
    if args.generate {
        let name = args.agent.as_deref().ok_or_else(|| {
            anyhow::anyhow!(
                "--generate requires an agent name: borgkit test --generate <AgentName>"
            )
        })?;
...
fn generate_test(project_dir: &Path, lang: &Lang, agent_name: &str) -> Result<()> {
    ...
    logger::dim(&format!("Run with: borgkit test {agent_name}"));
    Ok(())
}
...
fn run_tests(
    ...
    println!(
        "\n{}",
        format!("Running Borgkit tests ({lang}) — {label}").bold()
    );
...
fn ts_test_template(name: &str) -> String {
    format!(
        r#"/**
 * Unit tests for {name}
 * Generated by: borgkit test --generate {name}
 */
...
async def test_handle_request(agent, cap):
    req = AgentRequest(request_id="t1", from_id="borgkit://test",
...
async def test_unknown_capability(agent):
    req = AgentRequest(request_id="t2", from_id="borgkit://test",
