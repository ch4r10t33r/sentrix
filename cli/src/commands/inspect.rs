...
pub fn run(args: InspectArgs) -> Result<()> {
    match args.subcommand.as_deref() {
        Some("anr") => {
            let text = args
                .target
                .ok_or_else(|| anyhow!("Usage: inai inspect anr <anr-text>"))?;
        ...
        Some("agent") => {
            let id = args
                .target
                .ok_or_else(|| anyhow!("Usage: inai inspect agent <agent-id>"))?;
        ...
        None => {
            println!("\n{}", "inai inspect — subcommands".bold());
            println!();
            println!(
                "  {}  Decode an ANR record",
                "inai inspect anr <anr-text>  ".cyan()
            );
            println!(
                "  {}  List all mesh agents",
                "inai inspect agents           ".cyan()
            );
            println!(
                "  {}  Inspect one agent",
                "inai inspect agent <agent-id> ".cyan()
            );
            println!(
                "  {}  All capabilities on the mesh",
                "inai inspect capabilities     ".cyan()
            );
            println!();
            println!(
...
fn inspect_agent(id: &str, host: &str, port: u16, raw: bool) -> Result<()> {
        ...
        .and_then(|s| serde_json::from_str::<serde_json::Value>(s).ok())
    {
        if let Some(anr_text) = a["anr"].as_str() {
            ...
            println!();
            println!("  Decode with: inai inspect anr \"{}\"", anr_text);
        }
    }
    Ok(())
}
