/**
 * inai test — run unit tests for Inai agents.
 *
 * Usage:
 *   inai test                    # runs all tests in tests/
 *   inai test MyAgent            # runs tests/MyAgent.test.ts (or .py/.rs/.zig)
 *   inai test --generate MyAgent # scaffold a test file for MyAgent
 *
...
export async function testCommand(
  ...
  if (options.generate) {
    if (!agentName) {
      logger.error('--generate requires an agent name: inai test --generate MyAgent');
      process.exit(1);
    }
  ...
  logger.title(`Running Inai tests (${lang})`);
...
async function generateTestFile(
  ...
  logger.dim(`Run it with: inai test ${agentName}`);
}
 ...
 * Generated by: inai test --generate ${agentName}
 */
...
describe('${agentName}', () => {
  ...
  test('responds to __heartbeat capability', async () => {
    const req = {
      ...
      payload:    { senderId: 'inai://test', timestamp: Date.now() },
      timestamp:  Date.now(),
    };
    const resp = await agent.handleRequest(req as any);
...
async def test_heartbeat_capability(agent):
    req = AgentRequest(
        ...
        from_id="inai://test",
        capability="__heartbeat",
        payload={"senderId": "inai://test", "timestamp": 0},
        timestamp=0,
    )
    resp = await agent.handle_request(req)
