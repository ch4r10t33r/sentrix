/**
 * inai inspect — inspect ANR records and mesh topology.
 *
 * Subcommands:
 *   inai inspect anr <anr-text>            Decode and display an ANR record
 *   inai inspect agents [--host] [--port]  List all agents in the mesh
 *   inai inspect agent <agent-id>          Inspect a specific agent
 *   inai inspect capabilities              List all capabilities in the mesh
 */
...
export async function inspectCommand(
  ...
  switch (subcommand) {
    case 'anr':
      if (!target) {
        logger.error('Usage: inai inspect anr <anr-text>');
        process.exit(1);
      }
    ...
    case 'agent':
      if (!target) {
        logger.error('Usage: inai inspect agent <agent-id>');
        process.exit(1);
      }
    ...
    default:
      logger.title('inai inspect — available subcommands');
      console.log('');
      console.log('  inai inspect anr <anr-text>           Decode an ANR record');
      console.log('  inai inspect agents                   List all mesh agents');
      console.log('  inai inspect agent <agent-id>         Inspect one agent');
      console.log('  inai inspect capabilities             All mesh capabilities');
      console.log('');
      console.log('Options:');
  ...
  if (anrRaw.status === 'fulfilled') {
    try {
      ...
      if (anr.anr) {
        ...
        console.log(`    ${anr.anr}`);
        console.log('');
        console.log(`  Decode with: inai inspect anr "${anr.anr}"`);
      }
    } catch { /* */ }
  }
}
