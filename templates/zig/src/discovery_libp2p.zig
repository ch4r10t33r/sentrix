//! Kademlia DHT discovery for Inai — pure Zig, UDP/JSON transport.
//!
//! This is NOT libp2p wire-compatible. It uses JSON messages over UDP rather
//! than protobuf, and is designed to interoperate only with other Zig Inai
//! nodes running this same implementation.
//!
//! Key derivation matches the Rust `Libp2pDiscovery`:
//!   capability key : SHA-256("inai:cap:<capability>") → 64-hex chars
//!   ANR value key  : SHA-256("inai:anr:<agentId>")   → 64-hex chars
//!
...
pub const Libp2pDiscovery = struct {
    ...
    pub fn query(
        ...
        const cap_prefix = try std.fmt.allocPrint(self.allocator, "inai:cap:{s}", .{capability});
    ...
    pub fn findById(self: *Libp2pDiscovery, agent_id: []const u8) !?types.DiscoveryEntry {
        ...
        const anr_prefix = try std.fmt.allocPrint(self.allocator, "inai:anr:{s}", .{agent_id});
    ...
    fn announceEntry(self: *Libp2pDiscovery, entry: types.DiscoveryEntry, seq: u64) !void {
        ...
        const anr_prefix = try std.fmt.allocPrint(arena.allocator(), "inai:anr:{s}", .{entry.agent_id});
        ...
        for (entry.capabilities) |cap| {
            const cap_prefix = try std.fmt.allocPrint(arena.allocator(), "inai:cap:{s}", .{cap});
            const cap_key_hex = sha256Hex(cap_prefix);
