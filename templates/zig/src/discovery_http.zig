//! HttpDiscovery — REST client for a Inai-style central registry.
//!
...
//!
//! Environment:
//!   `INAI_DISCOVERY_URL` — base URL (optional trailing slash stripped)
//!   `INAI_DISCOVERY_KEY` — optional value for `X-Api-Key`
//!
...
pub const HttpDiscovery = struct {
    ...
    /// Returns null if `INAI_DISCOVERY_URL` is unset.
    pub fn fromEnv(allocator: std.mem.Allocator) ?HttpDiscovery {
        const url = std.process.getEnvVarOwned(allocator, "INAI_DISCOVERY_URL") catch return null;
        defer allocator.free(url);
        const key_owned = std.process.getEnvVarOwned(allocator, "INAI_DISCOVERY_KEY") catch null;
