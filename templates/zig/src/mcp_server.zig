//! MCP Server — Outbound bridge: Inai agent → MCP server
//!
//! Exposes any IAgent-compatible Inai agent as a Model Context Protocol
...
/// Expose a Inai agent as an MCP server.
///
...
pub fn serveAsMcp(
    ...
    const name = options.name orelse "inai-agent";
...
///   {"name":"cap","description":"Inai capability: cap",
...
fn buildToolsList(caps: []const []const u8, allocator: std.mem.Allocator) ![]const u8 {
    ...
    for (caps, 0..) |cap, i| {
        ...
        try desc_raw.writer().print("Inai capability: {s}", .{cap});
