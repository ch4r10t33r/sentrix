//! Google ADK → Inai Plugin (Zig) — HTTP Bridge
//!
...
pub const GoogleADKPlugin = struct {
    ...
    ///
    /// Extracts `message`, `input`, or `query` from the Inai payload; falls
    /// back to the raw JSON payload as the message text.
    ///
    /// A unique session ID is generated per call (sequential counter) so each
    /// Inai request is independently stateless from ADK's perspective.
    pub fn translateRequest(
        ...
        const session_id = try std.fmt.allocPrint(
            allocator, "inai-{s}-{d}", .{ req.request_id[0..@min(8, req.request_id.len)], seq },
        );
