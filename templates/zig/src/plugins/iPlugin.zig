//! Inai comptime plugin interface.
//!
//! A plugin wraps a "foreign" agent (one that does not natively speak the
//! Inai AgentRequest/AgentResponse wire format) and makes it callable via
...
//!       ![]const u8
//!       — convert a Inai AgentRequest to the foreign agent's native input
...
pub fn IPlugin(comptime TPlugin: type, comptime TAgent: type) type {
    return struct {
        ...
        ///   1. translateRequest  — Inai AgentRequest → foreign format
        ///   2. invokeNative      — run the wrapped agent
        ///   3. translateResponse — foreign output → Inai AgentResponse
        pub fn invoke(
            plugin: *TPlugin,
            agent: *TAgent,
