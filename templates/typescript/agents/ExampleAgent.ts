...
export class ExampleAgent implements IAgent {
  ...
  async registerDiscovery(): Promise<void> {
    ...
    // Honour INAI_DISCOVERY_TYPE for libp2p / onchain backends; the factory
    // already handles INAI_DISCOVERY_URL → http and defaults to local.
    const discoveryType = process.env['INAI_DISCOVERY_TYPE'];
  ...
  getAnr(): DiscoveryEntry {
    const host     = process.env['INAI_HOST'] ?? 'localhost';
    const port     = parseInt(process.env['INAI_PORT'] ?? '6174', 10);
    const tls      = (process.env['INAI_TLS'] ?? 'false').toLowerCase() === 'true';
  ...
  async checkPermission(caller: string, capability: string): Promise<boolean> {
    ...
    // 2. Capability-specific allow-list (opt-in via INAI_PERMITTED_CALLERS env var)
    //    Format: "cap1:caller1,caller2;cap2:caller3"
    const permittedRaw = process.env['INAI_PERMITTED_CALLERS'];
    if (permittedRaw) {
    ...
    //    Activated by INAI_REGISTRY_ADDRESS env var
    const registryAddress = process.env['INAI_REGISTRY_ADDRESS'];
    if (registryAddress) {
  ...
  async signMessage(message: string): Promise<string> {
    const privKeyHex = process.env['INAI_AGENT_KEY'];
    if (!privKeyHex) {
      throw new Error('signMessage: no signing key — set INAI_AGENT_KEY=<hex-private-key>');
    }
  ...
  private async _checkOnChainDelegation(
    ...
    const rpcUrl = process.env['INAI_RPC_URL'];
...
/**
 * Parse a INAI_PERMITTED_CALLERS string into a capability → caller-set map.
 *
 * Format:  "cap1:caller1,caller2;cap2:caller3"
...
//   INAI_PORT=9090 npx ts-node agents/ExampleAgent.ts
//
// Or via inai-cli:
