 ...
 *   Routing     : Kademlia DHT (custom /inai/kad/1.0.0 protocol)
 ...
 *   SHA256("inai:cap:<capability>") → CIDv1 → dht.provide() / findProviders()
 *
 * Full DiscoveryEntry is stored as a signed JSON envelope:
 *   SHA256("inai:anr:<agentId>") → key → dht.put() / get()
 *
...
export interface Libp2pDiscoveryConfig {
   ...
   * Also reads from INAI_BOOTSTRAP_PEERS env var (comma-separated).
   */
...
// ── Sentinel bootstrap peers for the public Inai network ──────────────────
// Replace these with real multiaddrs when production bootstrap nodes are deployed.
const INAI_BOOTSTRAP_PEERS: string[] = [
  // '/ip4/bootstrap1.inai.io/udp/4001/quic-v1/p2p/12D3KooWXXX...',
];
...
export class Libp2pDiscovery implements IAgentDiscovery {
  ...
  static async create(config: Libp2pDiscoveryConfig = {}): Promise<Libp2pDiscovery> {
    ...
    // ── Bootstrap peers ──────────────────────────────────────────────────────
    const envPeers    = process.env['INAI_BOOTSTRAP_PEERS']?.split(',').filter(Boolean) ?? [];
    const allBootstrap = [
      ...(config.bootstrapPeers ?? []),
      ...envPeers,
      ...INAI_BOOTSTRAP_PEERS,
    ];
    ...
    const node = await createLibp2p({
      ...
      services: {
        identify: identify(),
        dht: kadDHT({
          protocol:   '/inai/kad/1.0.0',  // isolated from public IPFS DHT
