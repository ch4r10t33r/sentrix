/**
 * DHT key derivation for the Inai capability registry.
 *
 ...
 *
 * Key schema:
 *   capability key  →  SHA256("inai:cap:<capability>")  → CIDv1
 *   anr value key   →  "/inai/anr/" + hex(SHA256("inai:anr:<agentId>"))
 *   pid→agentId key →  "/inai/pid/" + peerId.toString()
 */
...
export async function capabilityCid(capability: string): Promise<CID> {
  const input = new TextEncoder().encode(`inai:cap:${capability}`);
...
export async function anrDhtKey(agentId: string): Promise<Uint8Array> {
  const input  = new TextEncoder().encode(`inai:anr:${agentId}`);
  const hash   = await sha256.digest(input);
  const hex    = toString(hash.bytes, 'hex');
  return fromString(`/inai/anr/${hex}`);
}
...
export function pidDhtKey(peerIdStr: string): Uint8Array {
  return fromString(`/inai/pid/${peerIdStr}`);
}
