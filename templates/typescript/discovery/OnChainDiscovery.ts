...
export interface OnChainDiscoveryConfig {
  /** JSON-RPC endpoint. Default: INAI_RPC_URL env var */
  rpcUrl?: string;
  /** Deployed ERC-8004 registry contract address. Default: INAI_CONTRACT_ADDRESS env var */
  contractAddress?: string;
  /** Hex private key for signing transactions. Omit for read-only mode. Default: INAI_PRIVATE_KEY env var */
  privateKey?: string;
...
export class OnChainDiscovery implements IAgentDiscovery {
  ...
  constructor(cfg: OnChainDiscoveryConfig = {}) {
    const rpcUrl = cfg.rpcUrl
      ?? process.env['INAI_RPC_URL']
      ?? '';
    const contractAddress = cfg.contractAddress
      ?? process.env['INAI_CONTRACT_ADDRESS']
      ?? '';
    const privateKey = cfg.privateKey
      ?? process.env['INAI_PRIVATE_KEY']
      ?? '';

    if (!rpcUrl) {
      throw new Error(
        '[OnChainDiscovery] rpcUrl is required. Pass it in config or set INAI_RPC_URL.'
      );
    }
    if (!contractAddress) {
      throw new Error(
        '[OnChainDiscovery] contractAddress is required. Pass it in config or set INAI_CONTRACT_ADDRESS.'
      );
    }
  ...
  private assertReadWrite(operation: string): void {
    if (!this.rwContract) {
      throw new Error(
        ...
        'Pass it in config or set INAI_PRIVATE_KEY.'
      );
    }
  }
}
