/**
 * Inai HTTP Server (TypeScript)
 * ─────────────────────────────────────────────────────────────────────────────
 * Starts an Express HTTP server for a WrappedAgent, exposing the standard
 * Inai endpoints so agents can discover and call each other over the network.
 *
 * Endpoints
 * ─────────
...
export interface ServeOptions {
  ...
  host?: string;
  /** TCP port. Overridden by INAI_PORT env var if set. Default: 6174. */
  port?: number;
...
export async function serve(
  ...
  const port = parseInt(process.env.INAI_PORT ?? String(options.port ?? 6174), 10);
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const shutdown = async (signal: string): Promise<void> => {
      if (!options.silent) {
        process.stdout.write(`\n[Inai] ${signal} received — shutting down gracefully…\n`);
      }
      server.close(async () => {
        try {
