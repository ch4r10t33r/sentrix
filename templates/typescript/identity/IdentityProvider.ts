...
export class AnonymousIdentity implements IdentityProvider {
  ...
  agentId() { return `inai://agent/${this.name}`; }
 ...
 *
 * @example
 *   process.env.INAI_AGENT_KEY = '0xdeadbeef...';
...
export class EnvKeyIdentity implements IdentityProvider {
  ...
  constructor(envVar = 'INAI_AGENT_KEY', nameOverride?: string) {
    const val = process.env[envVar];
    if (val) {
      this._delegate = new RawKeyIdentity(val, nameOverride);
    } else {
      console.warn(
        `[Inai] ${envVar} not set — using anonymous identity. ` +
 ...
 *   console.log(identity.agentId()); // inai://agent/0x...
 */
export class LocalKeystoreIdentity implements IdentityProvider {
  ...
  constructor(
    ...
    const dir  = keystoreDir ?? path.join(os.homedir(), '.inai', 'keystore');
  ...
  private _loadOrCreate(dir: string, name: string): string {
    ...
    console.log(`[Inai] New identity created: ${keyfile}`);
    return key;
  }
}
 ...
 * All other Inai features work identically whether you use this mode or
 * an off-chain mode.
 *
...
export function identityFromConfig(config: IdentityConfig): IdentityProvider {
  ...
  switch (mode) {
    ...
    case 'env':       return new EnvKeyIdentity(envVar ?? 'INAI_AGENT_KEY', name);
    case 'raw':
