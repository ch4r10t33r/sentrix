//! OpenAI-compatible Chat Completions → Inai Plugin (Rust) — HTTP Bridge
//!
//! Wraps any OpenAI-compatible chat completions endpoint so it participates in
//! the Inai mesh as a standard `IAgent`.  Works with the official OpenAI API,
...
//! ── Usage ─────────────────────────────────────────────────────────────────────
//!
//!   use inai::plugins::openai::{OpenAIPlugin, OpenAIService};
...
//!       agent_id:     "inai://agent/gpt".to_string(),
...
#[async_trait]
impl InaiPlugin<OpenAIService> for OpenAIPlugin {
