...
/// Eliza, CrewAI, etc.) into the Inai mesh.
#[async_trait]
pub trait InaiPlugin<TAgent>: Send + Sync {
    /// Extract capability descriptors from the native agent.
    fn extract_capabilities(&self, agent: &TAgent) -> Vec<CapabilityDescriptor>;

    /// Translate a Inai `AgentRequest` into the native framework's input type
...
pub struct WrappedAgent<TAgent, P: InaiPlugin<TAgent>> {
    agent:        TAgent,
    plugin:       P,
    config:       PluginConfig,
...
impl<TAgent, P: InaiPlugin<TAgent>> WrappedAgent<TAgent, P> {
    ...
    P:      InaiPlugin<TAgent> + Send + Sync,
{
    ...
    async fn handle_request(&self, request: AgentRequest) -> AgentResponse {
        ...
        // 1. Translate Inai request → native input
