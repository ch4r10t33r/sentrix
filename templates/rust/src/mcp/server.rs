//! MCP Outbound Bridge — exposes any Inai `IAgent` as an MCP server.
//!
//! Receives MCP JSON-RPC 2.0 messages from an MCP client (e.g. Claude Desktop,
//! Cursor, Cline) and dispatches them to the wrapped `IAgent`.  Each Inai
...
//! ── Usage (stdio) ─────────────────────────────────────────────────────────────
//!
//!   use inai::mcp::{serve_as_mcp, ServeMcpOptions, Transport};
//!
...
fn build_tools_list<A: IAgent>(agent: &A) -> Value {
    let tools: Vec<Value> = agent
        ...
        .map(|cap| {
            json!({
                "name":        cap,
                "description": format!("Inai capability: {cap}"),
                "inputSchema": {
                    "type":       "object",
                    "properties": {
                        "payload": { "type": "object" }
    ...
    A: IAgent + Clone + Send + Sync + 'static,
{
    let server_name = options
        .name
        .clone()
        .unwrap_or_else(|| "inai-agent".to_string());
    ...
    A: IAgent + Clone + Send + Sync + 'static,
{
    ...
    println!("[inai-mcp] SSE server listening on http://{addr}");
    println!("[inai-mcp] Connect Claude Desktop to: http://{addr}/sse");
    ...
    A: IAgent + Clone + Send + Sync + 'static,
{
    ...
    let addr = format!("{host}:{port}");
    let listener = tokio::net::TcpListener::bind(&addr).await?;
    println!("[inai-mcp] HTTP server listening on http://{addr}");
    println!("[inai-mcp] MCP streamable endpoint: http://{addr}/mcp");
