...
/// Every Inai agent must implement this.
///
...
pub trait IAgent: Send + Sync {
    ...
    async fn sign_message(&self, message: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let raw_key = env::var("INAI_AGENT_KEY").map_err(|_| {
            "sign_message: no signing key — set INAI_AGENT_KEY=<hex-private-key> \
        ...
        let key_bytes = {
            ...
            hex_decode(key_hex).map_err(|_| "sign_message: INAI_AGENT_KEY is not valid hex")?
        };
        if key_bytes.len() != 32 {
            return Err("sign_message: INAI_AGENT_KEY must be 32 bytes (64 hex chars)".into());
        }
