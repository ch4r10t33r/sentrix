...
instance that can be registered, discovered, and called through Inai.
...
Lifecycle:
  ...
  4.         agent.handle_request(req)   # receives Inai calls

Plugin responsibilities:
  - extract_capabilities()   map framework tools → Inai capability names
...
class PluginConfig:
    ...
    agent_id:     str          = "inai://agent/unnamed"
...
class InaiPlugin(ABC, Generic[TAgent]):
    """
    Abstract base for all Inai framework adapters.
    ...
    def translate_request(
        ...
        """
        Convert a Inai AgentRequest into whatever the framework expects
    ...
    def translate_response(self, native_result: Any, request_id: str) -> AgentResponse:
        """
        Convert the framework's native result into a Inai AgentResponse.
        """
        ...
    ...
    def wrap(self, agent: TAgent) -> "WrappedAgent[TAgent]":
        """
        Wrap a framework-native agent in a Inai-compatible IAgent.
...
class WrappedAgent(IAgent, Generic[TAgent]):
    """
    An IAgent produced by InaiPlugin.wrap().
    Dispatches incoming Inai requests to the framework-native agent
    ...
    async def serve(self, host: str = "0.0.0.0", port: int = 6174) -> None:
        ...
        Args:
            ...
            port: TCP port.  Overridden by INAI_PORT env var if set.
...
def _print_startup_banner(agent: "WrappedAgent") -> None:
    ...
    line = f"{DIM}{'─' * 60}{W}"
    print(f"\n{line}")
    print(f"  {B}{C}Inai Agent Online{W}  {DIM}v{cfg.version}{W}")
    print(line)
