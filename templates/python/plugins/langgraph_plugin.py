...
in priority order:
  ...
  1. Explicit map in PluginConfig.capability_map
       { "inai_cap_name": "node_or_tool_name" }
...
─────
  ...
  config = LangGraphPluginConfig(
      ...
      # optional: expose each tool as a separate Inai capability
  ...
  # Inai will now call agent.handle_request(req)
...
class LangGraphPluginConfig(PluginConfig):
    ...
    # If True, each tool in the graph is exposed as a separate Inai capability.
...
class LangGraphPlugin(InaiPlugin):
    """
    Inai plugin for LangGraph agents.
    ...
    def translate_request(
        ...
        if native == "__graph__":
            ...
            return {
                ...
                "__inai_request_id__": req.request_id,
                "__inai_capability__": req.capability,
                "__inai_from__": req.from_id,
            }
        else:
    ...
    def translate_response(self, native_result: Any, request_id: str) -> AgentResponse:
        """
        Convert LangGraph's state dict output into a Inai AgentResponse.
