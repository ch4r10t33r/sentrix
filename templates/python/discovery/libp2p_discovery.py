"""
Libp2pDiscovery — fully P2P discovery backend for Inai.

Architecture:
  Transport   : QUIC (via inai-libp2p-sidecar Rust binary)
  Routing     : Kademlia DHT  (/inai/kad/1.0.0 — isolated from IPFS)
...
WHY A SIDECAR?
  ...
  1. A compiled `inai-libp2p-sidecar` binary (Rust, from templates/rust/)
  ...
  3. HttpDiscovery pointed at a local gateway (automatic fallback)
     ...
     Set INAI_LIBP2P_GATEWAY=http://localhost:7731 to enable.

  Configure via environment variables or Libp2pDiscoveryConfig:
    INAI_LIBP2P_SIDECAR   path to the compiled sidecar binary
    INAI_LIBP2P_NODE      path to the TypeScript entry point (if using Node)
    INAI_BOOTSTRAP_PEERS  comma-separated multiaddrs of bootstrap peers
    INAI_P2P_PORT         UDP port for QUIC listener (default: 0 = OS-assigned)
...
class Libp2pDiscoveryConfig:
    ...
    # Path to the compiled inai-libp2p-sidecar binary.
    # Falls back to INAI_LIBP2P_SIDECAR env var, then PATH lookup.
    sidecar_binary: Optional[str] = None

    # Path to the Node.js TypeScript entry point (fallback if no sidecar).
    # Falls back to INAI_LIBP2P_NODE env var.
...
class _SidecarProcess:
    """Manages a inai-libp2p-sidecar subprocess (Rust or Node.js)."""
    ...
    async def launch(cls, cfg: Libp2pDiscoveryConfig) -> '_SidecarProcess':
        ...
        if binary is None:
            raise RuntimeError(
                'inai-libp2p-sidecar not found.\n'
                'Options:\n'
                '  1. Compile the Rust sidecar: cd templates/rust && cargo build --release\n'
                '  2. Set INAI_LIBP2P_SIDECAR=/path/to/inai-libp2p-sidecar\n'
                '  3. Set INAI_LIBP2P_GATEWAY=http://localhost:7731 to use HTTP fallback\n'
                '  4. Set INAI_DISCOVERY_URL to use a centralised registry'
            )

        env_peers = os.environ.get('INAI_BOOTSTRAP_PEERS', '')
    ...
    def _resolve_binary(cfg: Libp2pDiscoveryConfig) -> Optional[str]:
        ...
        # 2. Env var
        env_path = os.environ.get('INAI_LIBP2P_SIDECAR')
        if env_path and os.path.isfile(env_path):
            return env_path
        # 3. PATH lookup
        found = shutil.which('inai-libp2p-sidecar')
        if found:
            return found
        # 4. Node.js fallback
        node_entry = cfg.node_entry or os.environ.get('INAI_LIBP2P_NODE')
...
class _HttpGatewayFallback(IAgentDiscovery):
    ...
    Set INAI_LIBP2P_GATEWAY=http://localhost:7731 to activate.
    """
...
class Libp2pDiscovery(IAgentDiscovery):
    """
    P2P discovery backend backed by a inai-libp2p-sidecar subprocess.

    Falls back to _HttpGatewayFallback if the sidecar is unavailable and
    INAI_LIBP2P_GATEWAY is set.
    """
    ...
    async def start(cls, cfg: Libp2pDiscoveryConfig = None) -> 'Libp2pDiscovery':
        ...
        try:
            ...
            gateway = os.environ.get('INAI_LIBP2P_GATEWAY')
            if gateway:
            ...
            raise RuntimeError(
                f'{e}\n\nAlternatively, set INAI_LIBP2P_GATEWAY=<url> to use an HTTP-to-DHT bridge.'
            ) from e
