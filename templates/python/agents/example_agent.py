...
class ExampleAgent(IAgent):
    ...
    async def register_discovery(self) -> None:
        ...
        discovery_type = os.environ.get('INAI_DISCOVERY_TYPE', 'libp2p')
    ...
    def _build_entry(self) -> 'DiscoveryEntry':
        import os
        from datetime import datetime, timezone
        host = os.environ.get('INAI_HOST', 'localhost')
        port = int(os.environ.get('INAI_PORT', '6174'))
        tls  = os.environ.get('INAI_TLS', 'false').lower() == 'true'
...
#   INAI_PORT=9090 python agents/example_agent.py
#
# Or via inai-cli:
