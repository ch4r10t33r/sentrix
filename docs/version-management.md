...
Inai follows **Semantic Versioning 2.0.0** (`MAJOR.MINOR.PATCH`).
...
writeConfig({ version: VERSION, … });     // inai.config.json
```
...
```bash
inai --version       # short: prints "0.1.0"
inai -v              # alias

inai version         # detailed build info:
#   Version    : 0.1.0
