...
Inai is a **decentralised agent networking framework** that lets you build, discover, and interconnect software agents without relying on any centralised service.
...
Every agent in Inai has a verifiable identity (backed by a secp256k1 key pair), a list of capabilities it exposes, and a signed network record — the **ANR** — that encodes all of this into a compact, portable string.
...
Switch between them with a single environment variable (`INAI_DISCOVERY_URL`) or a one-line config change — **your agent code never changes**.
...
- Every agent has an `agentId` URI (`inai://agent/<address>`)
