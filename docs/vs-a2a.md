# Inai vs A2A — What Problem Does Each Solve?

> **Short answer:** A2A defines *how* two agents talk. Inai defines *how agents find each other, prove who they are, establish trust, and transact* — problems A2A explicitly leaves out of scope.

They are more complementary than competing. A2A is a communication standard; Inai is coordination infrastructure. But if you are building an open, permissionless agent mesh without existing enterprise infrastructure, Inai solves problems A2A deliberately does not.
...
Inai fills exactly those gaps. Here is a precise breakdown:
...
**Inai:** Every agent announces itself to a Kademlia DHT (capability key = `SHA-256("inai:cap:<capability>")`). Any node can find any agent by capability with no prior knowledge of its URL, domain, or IP. mDNS handles LAN discovery. No admin, no registry, no URL exchange required.

```bash
# An agent deployed anywhere on the internet is immediately findable:
inai discover --capability translate
...
**Inai:** Every agent has a `did:key` derived from a secp256k1 or X25519 keypair. The DID is:
...
The Agent Network Record (ANR) — Inai's equivalent of an Agent Card — is signed by the agent's DID key. Recipients can verify it.
...
**Inai:** DIDComm v2 provides end-to-end encrypted, authenticated messages between agents:
...
**Inai:** Two complementary payment paths:
...
- **MPP (Machine Payments Protocol)** — explicit first-class support via the **MPP plugin** in Inai templates ([mpp.dev](https://mpp.dev)): HTTP **402** payment-required responses, challenge–credential–receipt flow, with **Tempo** stablecoin, **Stripe** Secure Payment Tokens (SPT), or **Lightning** depending on configuration. Ship-ready in **TypeScript**, **Rust**, and **Zig** scaffolds; Python template support is planned.
...
**Inai:** Agents connect to the libp2p mesh via QUIC. Circuit relay allows agents behind NAT or firewalls to be reachable. The same mesh protocol works on a LAN (mDNS) and across the open internet (DHT + relay).
...
**Inai:** You query by capability. The mesh returns the best available agents that provide it, with health scores and latency. The client picks one (or fans out to several). This enables:
...
**Inai:** Zig templates implement the full mesh protocol (discovery, invocation, gossip, DIDComm) using only `std.net` and `std.crypto` — no HTTP framework, no runtime dependencies. Agents can run on embedded hardware with kilobytes of RAM.
...
Inai handles **discovery and identity**; A2A handles **task communication**. A Inai agent can expose an A2A-compatible `/invoke` endpoint — letting it be discovered via Inai's DHT and invoked using the A2A task protocol. The two layers are orthogonal.

```
Inai DHT          → finds the agent's multiaddr + DID
