...
Inai's discovery layer is **pluggable** — you swap backends without changing a single line of agent code. This document describes each adapter, when to use it, and how the `DiscoveryFactory` selects between them.
...
**Enable with:** `INAI_DISCOVERY_TYPE=local`
...
**Backend:** any HTTP server that implements the Inai registry REST API
...
```bash
export INAI_DISCOVERY_URL=https://registry.example.com
export INAI_DISCOVERY_KEY=my-api-key   # optional
...
2. INAI_DISCOVERY_TYPE env var  →  local | http | libp2p
...
Do you need in-process discovery only (unit tests, single-process demos)?
  → LocalDiscovery   (INAI_DISCOVERY_TYPE=local — no ports opened)

Do you have a managed registry or are running in an enterprise network?
  → HttpDiscovery    (INAI_DISCOVERY_TYPE=http + INAI_DISCOVERY_URL=…)
...
|---|---|---|
| `INAI_DISCOVERY_TYPE` | Force a specific mode | `local` \| `http` \| `libp2p` |
| `INAI_DISCOVERY_URL` | HTTP registry URL (activates `http` if `TYPE` unset) | `https://registry.example.com` |
| `INAI_DISCOVERY_KEY` | API key for HTTP registry | `sk-…` |
| `INAI_AGENT_KEY` | 64-hex secp256k1 key for stable Peer ID | `openssl rand -hex 32` |
| `INAI_BOOTSTRAP_PEERS` | Comma-separated multiaddrs for libp2p | `/ip4/1.2.3.4/tcp/6174/p2p/12D3…` |
...
INAI_AGENT_KEY=<your 64-hex secp256k1 private key>
```

If `INAI_AGENT_KEY` is omitted, a random ephemeral key is generated each startup — the Peer ID will differ between runs.

### "The agent says discovery type is local but I set INAI_DISCOVERY_URL"

`INAI_DISCOVERY_URL` activates `http` discovery, not `libp2p`. To activate libp2p you need `INAI_DISCOVERY_TYPE=libp2p` (the URL is unused in libp2p mode). The DiscoveryFactory priority is:

```
1. Explicit type  →  INAI_DISCOVERY_TYPE=http|libp2p
2. URL present    →  INAI_DISCOVERY_URL set  →  http discovery
3. Default        →  local
```
...
- **`http` mode** — both agents must point to the same `INAI_DISCOVERY_URL`.
- **`libp2p` mode** — agents need at least one shared bootstrap peer. Set `INAI_BOOTSTRAP_PEERS=/ip4/<peer-ip>/tcp/6174/p2p/<PeerId>` or use mDNS on the same LAN (automatic, no config needed).
