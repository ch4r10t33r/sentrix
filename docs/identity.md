...
Inai agents need an identity to:
...
| `local` | no | no | **Default** — persistent key auto-created in `~/.inai/keystore/` |
...
```python
# Simplest: auto-creates key in ~/.inai/keystore/my-agent.key
from identity.provider import LocalKeystoreIdentity

identity = LocalKeystoreIdentity(name="my-agent")
print(identity.agent_id())   # inai://agent/0xAbCd...
...
# export INAI_AGENT_KEY=0x<32-byte-hex>
...
console.log(identity.agentId());  // inai://agent/0xAbCd...
...
## How identity flows through Inai
...
export INAI_AGENT_KEY=0xa1b2c3d4e5f6...  # with 0x
export INAI_AGENT_KEY=a1b2c3d4e5f6...    # also accepted
```
...
# Or let Inai create one automatically:
